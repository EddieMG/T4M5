...
def yield_mask_dicts(img_dir, lbl_path):
    ...
    for img_path in img_dir.iterdir():
        ...
        for obj in objs[idx]:
            if obj.class_id != 10:
                ...
                annotation = {
                        ...
                        "segmentation": seg,
                        #"category_id": 2 if obj.class_id == 1 else 0,
                        "category_id": COCO_CATEGORIES[obj.class_id],
                        "iscrowd": 0,
                    }
        ...
        if str(img_path) == "/home/mcv/datasets/KITTI-MOTS/training/image_02/0000/000151.png":
            print("*****************************************************************")
            print(record)
            print("*****************************************************************")
        #print("---")
            ...
            "annotations": [
                {
                    "bbox": to_XYXY(toBbox(obj.mask)).tolist(),
                    "bbox_mode": BoxMode.XYXY_ABS,
                    "segmentation": list(filter(lambda p: len(p) >= 6, (poly.reshape(-1) for poly in Mask(_mask.decode([obj.mask])).polygons().points))),
...
if __name__ == "__main__":
    ...
    MetadataCatalog.get("kitti-mots").set(thing_classes=list(KITTI_CATEGORIES.keys()))
