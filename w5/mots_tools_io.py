...
def load_sequences(path, seqmap):
    ...
    for seq in seqmap:
        ...
        if os.path.isdir(seq_path_folder):
            objects_per_frame_per_sequence[seq] = load_images_for_folder(
                seq_path_folder
            )
...
def filename_to_frame_nr(filename):
    assert (
        len(filename) == 10
    ), "Expect filenames to have format 000000.png, 000001.png, ..."
...
def load_image(filename, id_divisor=1000):
    ...
    objects = []
    mask = np.zeros(
        img.shape, dtype=np.uint8, order="F"
    ...
    for idx, obj_id in enumerate(obj_ids):
        ...
        objects.append(
            SegmentedObject(rletools.encode(mask), obj_id // id_divisor, obj_id)
        )
        ...
        for t, objects in frames.items():
            for obj in objects:
                print(
                    ...
                    obj.mask["size"][0],
                    obj.mask["size"][1],
                    obj.mask["counts"].decode(encoding="UTF-8"),
                    file=f,
                )